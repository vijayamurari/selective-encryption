...
key=np.load('rgbchvideokey_swap.npy')
for i in range(len(files1)):
    ...
    encrypted_image_blue1[:,640:1280]=enc_blue_ch[:,0:640]
    encrypted_image_blue1[:,0:640]=enc_blue_ch[:,640:1280]

    encrypted_image_green1=np.zeros((r,c),np.uint8)
    encrypted_image_green1[:,640:1280]=enc_green_ch[:,0:640]
    encrypted_image_green1[:,0:640]=enc_green_ch[:,640:1280]

    encrypted_image_red1=np.zeros((r,c),np.uint8)
    encrypted_image_red1[:,640:1280]=enc_red_ch[:,0:640]
    encrypted_image_red1[:,0:640]=enc_red_ch[:,640:1280]
    
    decrypted_image_blue=np.zeros((r, c), np.uint8)
    decrypted_image_green=np.zeros((r, c), np.uint8)
    decrypted_image_red=np.zeros((r, c), np.uint8)
    for row in range(r):
    ...
    cv2.imwrite("G:/impl/linux_backup/dec_rgb_swap/frame%d.png" %i,img)
